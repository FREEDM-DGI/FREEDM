...
\begin{longtable}{|p{3cm}|p{4cm}|p{10cm}|c|}
    ...
    Failure3 & Fail after sending the DGI device states. & SST (0) net 0.0 \newline SST (1) net 5.0 \newline SST (0) net 0.0 & \\
    Failure4 & Fail after sending the device states and restart instantly. & SST (0) net 0.0 \newline SST (1) net 5.0 & \\
    Failure5 & Fail after sending the device states and restart after a delay. & SST (0) net 0.0 \newline SST (1) net 5.0 \newline SST (0) net 0.0 \newline SST (1) net 5.0 & \\
    ...
    UnexpectedError6 & Send an unrecognized packet header to the DGI. & & \\
    UnexpectedError7 & Send a single packet with the wrong delimiter to the DGI session port. & & \\
    UnexpectedError8 & Send a single packet with the wrong delimiter to a plug and play adapter. & & \\
    UnexpectedError9 & Send a packet with the wrong delimiter followed by a real packet to the session protocol. & & \\
    UnexpectedError10 & Send a packet with the wrong delimited followed by a real packet to a plug and play adapter. & & \\
