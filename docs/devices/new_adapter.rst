...
This block of code executes when a device is seen for the first time in the *adapter.xml* configuration file, and creates a virtual device for use in the DGI. The name of this device is already associated with your adapter through the :cpp:function:`void IAdapter::RegisterDevice(std::string)` function call, and if you only need to know the number of devices you can overwrite this function in your implementation file. However, by default, the type is not sent to your adapter. Change the code to the following::
...
With this modification, :cpp:function:`void CYourAdapter::RegisterType(std::string)` will be called once for each device associated with your adapter in the *adapter.xml* specification file. However, you will have to implement the RegisterType function as it is not a standard adapter function.
